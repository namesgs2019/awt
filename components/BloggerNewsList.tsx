...
export function BloggerNewsList({ posts }: { posts: NewsListPost[] }) {
  ...
  return (
    <div className="insights-browser">
      ...
      {filteredPosts.length === 0 ? <p className="insights-state">News posts are being updated. Please check back soon.</p> : null}

      <div className="insights-list">
        {displayedPosts.map((post) => (
          <article className="insight-card blogger-news-card" key={post.id}>
            <Link className="insight-card-main" href={`/insights/news/${post.slug}/`}>
              {post.thumbnail ? (
              ...
              <div>
                ...
                <p>{post.excerpt || "Latest news and updates from Advanced Water Technology."}</p>
